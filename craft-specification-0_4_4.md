...
**Exit boundary declaration.** A chain whose output is consumed by a party or system outside the chain must declare what that output carries on the way out, so that a consumer can determine from the output alone what it claims and what would show it wrong. The declaration names the exit obligations the chain operates under: the graded profile of the support the output rests on, the conditions under which the output would be overturned stated in terms of what could be found in the world, the disagreement its support contained rather than an average that conceals it, the derivation of that support to origin with each rung labeled, and the judgment of sufficiency left to the consuming party. These obligations are satisfied by adopting STRUCK, the companion specification at the exit boundary, or by declaring equivalent obligations with a documented equivalence argument, under the same discipline Section 8 applies to condition omission; an equivalent realization is certified independently against STRUCK, is not weaker than adopting STRUCK, and enumerates the exit realizations it permits rather than leaving the path open. A chain whose output leaves it and carries no exit boundary declaration has left the output stage outside its own conformance surface; the declaration is required for specification compliance under either realization. A chain whose evaluation is coupled to its intervention so that no output travels to an external consuming system, the coupled evaluation-intervention structure specified in Condition 5, declares that instead, and the declaration is complete.
...
**CRAFT and WALKRI validate each other (the mutual loop).** The signal above is one half of a mutual relationship. CRAFT validates that WALKRI's own specification is itself a legitimate evaluation chain, carrying a decision context, an ontology, measurement instruments, pre-specified criteria, decision logic, and a feedback mechanism, so WALKRI is not exempt from the scrutiny it helps a domain apply. WALKRI validates that a CRAFT decomposition into a domain produced well-formed instruments: a decomposition whose capture points fail WALKRI's instrument quality has not been correctly decomposed. Neither is exempt from the other's scrutiny, which is what keeps the pair from being gamed. The canonical statement of this mutual loop is the interaction-architecture document; this is CRAFT's declaration of its half. Activity profile trigger: when WALKRI's specification is revised, CRAFT re-validates it as a chain; when a CRAFT decomposition is produced or revised, WALKRI validates its instruments.
...
v0.4.6 (2026-08-20): The CRAFT-WALKRI mutual-validation loop declared in Section 11. Section 11 previously carried only the upward per-axis-quality-to-CRAFT signal; it now also declares CRAFT's half of the mutual loop, that CRAFT validates WALKRI's own specification as a legitimate evaluation chain (WALKRI's self-application, closed in WALKRI v0.2.1) and WALKRI validates that a CRAFT decomposition produced well-formed instruments, neither exempt from the other. The canonical statement lives in the interaction-architecture document; this is CRAFT's own-face declaration of its half. From the 2026-08-20 CRAFT-WALKRI coherence pass, which also corrected a satisfies-versus-inherits slide in WALKRI Part XI (WALKRI conforms to CRAFT's instrument-facing conditions, its content descends from Precision-First). The six conditions are unchanged in count; no condition or construction-grammar requirement changed.

v0.4.5 (2026-08-03): Exit boundary declaration added to Section 6.1, mirroring the ingestion boundary declaration. A chain whose output is consumed by a party or system outside the chain must declare what that output carries on the way out, satisfied by adopting STRUCK (the companion specification at the exit boundary) or by declaring equivalent obligations with a documented equivalence argument, the equivalence path hardened (certified independently against STRUCK, not weaker than adopting STRUCK, permitted realizations enumerated rather than left open). A chain whose evaluation is coupled to its intervention so that no output travels to an external consuming system declares that instead. This closes the symmetric gap that CRAFT named the ingestion boundary while leaving the exit boundary a silent default: a chain could be compliant while what its output owed on the way out was never declared. The requirement is realization-open, naming the obligation rather than mandating the instrument or its realization, consistent with the meta-standard level; the exit-boundary realization question (on-the-face versus bound-and-reachable) lives in STRUCK and is not decided here. The six conditions are unchanged in count. This makes concrete the settled relationship that CRAFT requires both ORE and STRUCK while neither requires CRAFT.
