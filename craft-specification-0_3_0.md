...
Six structural elements must be specified at this stage and carried into every downstream stage of the chain:
...
A construct-type claim is one whose object is abstract and not directly observable; it requires systematization and proxy specification before measurement can proceed. For construct-type chains, a nomological network declaration is required at this stage: naming the construct, its sub-constructs, the observable criteria that proxy for each sub-construct, and the expected relationships between them. The nomological network declaration has three valid states: established (with an evidence reference), partial (with named gaps declared explicitly), or absent-declared (explicitly stated as a current gap). A declared-partial or absent-declared nomological network is a legible gap that an attesting party can evaluate; an undeclared absence is not legible and is not a valid operating state.

For composite claims: when the chain's primary claim aggregates multiple sub-claims into a composite score or output, the aggregation method is part of the claim object declaration. The aggregation method must specify what sub-claims are combined, how they are weighted or ordered, and what the composite score is claimed to represent. A composite claim without a declared aggregation method is an under-specified claim object, regardless of how well the individual sub-claims are specified.
...
**Independent attestation.** Each specification document must be attested by a party independent of the operating organization before data collection begins. The attesting party must confirm that the specification is internally consistent, that each document is derivable from its prior stage, that the evaluation criteria specify conditions under which data would be rejected, and that the risk-bearer identification declared in Condition 1 is accurate and complete.

An attesting party that has a financial or organizational interest in the evaluation's outcome does not satisfy the independence requirement.
...
### 7.2 Independent attestation

Independent attestation is the structural answer to the specification gaming problem: the possibility that an organization writes its own specifications to always pass because it controls both the specification and the evaluation. Independence requires that the attesting party:

Has no financial or organizational interest in the evaluation's outcome.

Reviews the specification for genuine falsifiability, not merely for formal completeness.

Reviews the risk-bearer identification declared in Condition 1 for accuracy and completeness.

Assesses whether the declared adversarial threat model is realistic given the chain's domain and specification.

Assesses whether the deployment scale context declaration is consistent with the chain's feedback protocol requirements.

Documents the basis for their attestation, including any conditions they required the operator to address before attesting.

Is identified in the attestation record and has reputational exposure for the attestation. An anonymous attestation does not satisfy this requirement.

[Needs validation: what existing attestation infrastructure in the relevant domains (blockchain, grants evaluation, financial audit, program evaluation) satisfies or could be adapted to satisfy the independence requirement? This is a domain-application question that each CRAFT implementation must address specifically.]
...
`condition_findings`: for each condition evaluated, the result and the basis. For rejections, the specific condition that triggered rejection, the data values at evaluation time, and the layer attribution (`craft_base`, `domain_application`, or `per_axis_quality`).

`gate_check_result`: the adversarial gate result at evaluation time.

`propagation_targets`: for rejection findings, the prior condition(s) that the rejection signal must propagate to under Condition 6.
